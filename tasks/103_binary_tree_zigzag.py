...
class Solution:
    def zigzagLevelOrder(self, root: Optional[TreeNode]) -> list[list[int]]:
        ...
        next_level = []
        result = []
        next_level_direction_right = False

        while len(cur_level) > 0:
            for node in cur_level[::-1]:
                child_nodes = [node.left, node.right] if next_level_direction_right else [node.right, node.left]
            ...
            next_level = []
            next_level_direction_right = not next_level_direction_right
